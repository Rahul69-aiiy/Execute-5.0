...
const WasteOperations = () => {
    ...
    const fetchWasteData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/dashboard/waste-operations');

        if (response.data.success) {
          const { weeklyGeneration, binStatus, recommendation, aiRecommendations, communityScore } = response.data.data;
          setWasteData({
            weeklyGeneration: weeklyGeneration || wasteData.weeklyGeneration,
            binStatus,
            recommendation,
            aiRecommendations,
            communityScore
        ...
        console.error('Error fetching waste operations data:', error);
      } finally {
        setLoading(false);
      }
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 h-[400px] bg-white rounded-2xl border border-slate-200 shadow-sm p-2">
                    <ForecastingChart
                        ...
                        labels={wasteData.weeklyGeneration.labels}
                        historicalData={wasteData.weeklyGeneration.actual}
                        forecastData={wasteData.weeklyGeneration.forecast}
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
                    ...
                    <div className="space-y-4">
                        {wasteData.binStatus.length > 0 ? (
                          wasteData.binStatus.map((bin, idx) => (
                            <div key={idx} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="flex justify-between items-center mb-1">
                                    <span className="text-sm text-slate-600">{bin.location}</span>
                                    <span className={`text-[10px] ${bin.status === 'high' ? 'bg-rose-100 text-rose-600' : 'bg-green-100 text-green-600'} px-2 py-0.5 rounded font-bold uppercase`}>
                                      {bin.urgency}
                                    </span>
                                </div>
                                <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                                    <div
                                      className={`h-full ${bin.status === 'high' ? 'bg-rose-500' : 'bg-green-500'}`}
                                      style={{ width: `${bin.level}%` }}
                                    ></div>
                                </div>
                                <p className="text-xs text-right text-slate-400 mt-1 font-bold">Bin at {bin.level}%</p>
                            </div>
                          ))
                        ) : (
                          <>
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-100">
                            ...
                            <p className="text-xs text-slate-500 mt-1">
                                {wasteData.recommendation || 'Route Truck #2 to Mess Hall A immediately. Delay Hostel C collection.'}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <div className="space-y-3 relative z-10">
                        {wasteData.aiRecommendations.length > 0 ? (
                          wasteData.aiRecommendations.map((rec) => (
                            <div
                              key={rec.id}
                              className={`flex gap-3 items-start p-3 ${rec.priority === 'high' ? 'bg-purple-50 border-purple-100' : 'bg-slate-50 border-slate-100'} rounded-xl border`}
                            >
                                <div className="mt-1">
                                    <div className={`w-2 h-2 rounded-full ${rec.priority === 'high' ? 'bg-purple-600 animate-ping' : 'bg-slate-400'}`}></div>
                                </div>
                                <div>
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        {rec.actionType === 'auto' ? (
                                          <button className={`px-3 py-1 rounded-md ${rec.priority === 'high' ? 'bg-purple-600 text-white' : 'bg-slate-600 text-white'} text-xs font-bold hover:opacity-90 transition-colors`}>
                                              {rec.action}
                                          </button>
                                        ) : (
                                          <>
                ...
                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
                    ...
                    <div className="text-center p-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl border border-green-100">
                        <h2 className="text-5xl font-bold text-green-600 mb-2">
                          {wasteData.communityScore?.grade || 'A+'}
                        </h2>
                        <p className="text-slate-800 font-bold">Sustainability Score</p>
                        <p className="text-xs text-slate-500 mt-2">
                            Top performing building: <span className="text-green-600 font-bold">
                              {wasteData.communityScore?.topBuilding || 'Library Block'}
                            </span>
                        </p>
                        ...
                        <div className="flex items-end justify-center gap-2 h-16 mt-2">
                            <div
                              className="w-8 bg-slate-200 rounded-t-sm"
                              style={{ height: `${wasteData.communityScore?.rankings?.[0] || 60}%` }}
                            ></div>
                            <div
                              className="w-8 bg-gradient-to-t from-green-500 to-emerald-400 rounded-t-sm shadow-lg relative"
                              style={{ height: `${wasteData.communityScore?.rankings?.[1] || 100}%` }}
                            >
                                <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-bold text-green-600 whitespace-nowrap">YOU</span>
                            </div>
                            <div
                              className="w-8 bg-slate-200 rounded-t-sm"
                              style={{ height: `${wasteData.communityScore?.rankings?.[2] || 80}%` }}
                            ></div>
                        </div>
                    </div>
                </div>
